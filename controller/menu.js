...
const editCatagories = async (req, res) => {
    try {
        ...
        await Menu_categ.update({ menu_categ_nm: req.body.editCatagoriesFrom.menu_categ_nm, rank: +req?.body?.editCatagoriesFrom.rank || 0 }, { where: { id: req.body.editCatagoriesFrom.id, hotel_id: req.user } })
        // setImmediate(() => {
...
const menuByCategory = async (req, res) => {
    try {
        ...
        return res.status(STATUSCODE.SUCCESS).json(success(MESSAGE.SUCCESS, { menu, restaurantDetails: { currency: hotel.currency, restaurantName: hotel.hotel_name, img: hotel.hotel_logo, address: hotel.address1 + "" + hotel.address2 } }, STATUSCODE.SUCCESS))
    } catch (err) {
