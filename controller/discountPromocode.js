...
const createPromoCode = async (req, res) => {
    try {
        ...
        const checkAvailablePromoCode = await PromoCode.findOne({ where: { promo_code, status: true } })
...
const updatePromoCode = async (req, res) => {
    try {
        ...
        const checkAvailablePromoCode = await PromoCode.findOne({ where: { promo_code, status: true, id: { [Op.ne]: id } } })
        if (checkAvailablePromoCode) return res.json(error("Promo Code Already Available", STATUSCODE.BAD_REQUEST))
